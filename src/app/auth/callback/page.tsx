  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        ...
        // Trigger Supabase to process the OAuth callback from the URL
        console.log('Processing OAuth callback from URL...');
        
        // Wait for Supabase to process the OAuth hash/parameters
        ...
        if (data.session?.user) {
          console.log('Session found immediately after processing!', data.session.user.email);
          subscription.unsubscribe();
          await processUserProfile(data.session.user, userType, isNewsletterSubscriber);
        } else {
          console.log('No immediate session after processing, waiting...');
          
          // Attempt to refresh or wait for session
          let attempts = 0;
          const maxAttempts = 5;
          ...
          while (attempts < maxAttempts) {
            ...
            if (retryData.session?.user) {
              ...
              subscription.unsubscribe();
              await processUserProfile(retryData.session.user, userType, isNewsletterSubscriber);
              return;
            }
            attempts++;
          }
