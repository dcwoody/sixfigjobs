    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {jobs?.map((job: Job) => (
            <Link key={job.JobID} href={`/jobs/${job.slug}`} className="block bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                ...
                {job.CompanyLogo && <Image src={job.CompanyLogo} alt={`${job.Company} logo`} width={40} height={40} className="w-10 h-10 object-contain ml-4" />}
              </div>
              <p className="text-sm text-gray-700 mb-4 line-clamp-3">{job.ShortDescription}</p>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-green-600">{job.formatted_salary || 'Salary not listed'}</span>
